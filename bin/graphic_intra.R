...
# Usage:
# ./graphic.R intraMerged.result OUTPUTname
#
# Output files: intraMerged_junction.result and OUTPUTname.pdf 
...
if(inputToolsNum >1)
{  
  ## generate intraMerged_junction.result ##
  input.out <- c()
  input.RS <- rowSums(1*(input[,-c(1:7)] >0))
  for (i in 1:inputEventNum)
  {
    E.one <- as.numeric(input[i,-c(1:7)])
    Jmed <- median(E.one)
  ...
  write.table(input.out, file="intraMerged_junction.result", append=FALSE, 
